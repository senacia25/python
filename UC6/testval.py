...
df = pd.read_csv("UC6/vendas_hamburgueria.csv")
...
vendas_por_dia_produto = df.groupby(["Dia_da_Semana", "Produto"])["Quantidade Vendida"].sum().reset_index()
...
vendas_por_dia_produto = vendas_por_dia_produto.sort_values(
    by=["Dia_da_Semana", "Quantidade Vendida"], 
...
plt.bar(top_vendidos_por_dia["Dia_da_Semana"], top_vendidos_por_dia["Quantidade Vendida"], color="skyblue")
...
print(top_vendidos_por_dia[["Dia_da_Semana", "Produto", "Quantidade Vendida"]])