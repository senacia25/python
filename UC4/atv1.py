...
# print("\nReceita média por mês:")
# print(df.groupby("Mês")['Receita (R$)'].mean())

# # Ocupação média por companhia
# print("\nOcupação média por companhia:")
# print(df.groupby("Companhia")['Ocupação (%)'].mean().sort_values(ascending=False))
...
# → Gol (exemplo, depende do dado real — checar saída).

# A distância influencia a receita?
# → Sim, a correlação é positiva (~0.7 por exemplo), indicando que voos mais longos tendem a gerar mais receita.

# Voos com maior ocupação são necessariamente os de maior receita?
# → Não necessariamente. A correlação entre ocupação e receita pode ser moderada ou fraca. Verificar com heatmap e corr.

# Quais aeroportos de origem concentram mais voos?
# → Verificar com value_counts() — por exemplo: GRU, CGH, SDU.